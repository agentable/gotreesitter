...
func TestInferredPythonTagsQueryCapturesOnlyCallableName(t *testing.T) {
	entry := DetectLanguageByName("python")
	if entry == nil {
...
`))
	...
	if got, want := strings.Join(definitions, ","), "Helper,leaf,run"; got != want {
		t.Fatalf("definition names = %q, want %q; tags=%+v", got, want, tags)
	}
...
func TestInferredLuaTagsQueryCapturesTopLevelFunctionsAndBareCalls(t *testing.T) {
	entry := DetectLanguageByName("lua")
	if entry == nil {
		t.Fatal("expected Lua language entry")
	}
	tagger, err := gotreesitter.NewTagger(entry.Language(), ResolveTagsQuery(*entry))
	if err != nil {
		t.Fatalf("NewTagger: %v", err)
	...
	tags := tagger.Tag([]byte(`
local function benchmark_lua_leaf()
  return "leaf"
end

local function benchmark_lua_run()
  benchmark_lua_leaf()
  helper.leaf()
end
`))
	...
	if got, want := strings.Join(definitions, ","), "benchmark_lua_leaf,benchmark_lua_run"; got != want {
		t.Fatalf("definition.function names = %q, want %q; tags=%+v", got, want, tags)
	}
	if got, want := strings.Join(calls, ","), "benchmark_lua_leaf"; got != want {
		t.Fatalf("reference.call names = %q, want %q; tags=%+v", got, want, tags)
	}
}
...
func TestInferredCTagsQueryCapturesPointerReturnFunction(t *testing.T) {
	...
	source := []byte(`const char *leaf(void) {
	return "leaf";
}
...
func TestInferredCPPTagsQueryCapturesClassMethodAndReceiverCall(t *testing.T) {
	entry := DetectLanguageByName("cpp")
	if entry == nil {
		t.Fatal("expected C++ language entry")
	}
	...
	source := []byte(`class Helper {
public:
	const char *leaf() { return "leaf"; }
};

const char *run(Helper *helper) {
	return helper->leaf();
}
`)
	parser := gotreesitter.NewParser(entry.Language())
	...
	tags := tagger.TagTree(tree)
	var classes, methods, functions, calls []string
	for _, tag := range tags {
		switch tag.Kind {
		case "definition.class":
			classes = append(classes, tag.Name)
		case "definition.method":
			methods = append(methods, tag.Name)
		case "definition.function":
			functions = append(functions, tag.Name)
		case "reference.call":
			calls = append(calls, tag.Name)
	...
	if got, want := strings.Join(classes, ","), "Helper"; got != want {
		t.Fatalf("definition.class names = %q, want %q; tags=%+v", got, want, tags)
	}
	if got, want := strings.Join(methods, ","), "leaf"; got != want {
		t.Fatalf("definition.method names = %q, want %q; tags=%+v", got, want, tags)
	}
	if got, want := strings.Join(functions, ","), "run"; got != want {
		t.Fatalf("definition.function names = %q, want %q; tags=%+v", got, want, tags)
	}
	if got, want := strings.Join(calls, ","), "leaf"; got != want {
		t.Fatalf("reference.call names = %q, want %q; tags=%+v", got, want, tags)
